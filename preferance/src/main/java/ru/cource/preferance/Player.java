...
public class Player {
    ...
    int suit; //какую масть игроку стоит выбрать козырем
    int nonBribe;
    String name;
    ...
    int as(int n, int suitBribe){ //проверим, является ли карта продолжением регрессии, начиная с высшей(туза)
        n++;
        if (n >= hand.size())
            return suitBribe;
    ...
    void as(int n){ //проверим, является ли карта продолжением прогрессии, начиная с низшей(семерки)
    ...
    public void prikup(int m){
        if (condition < 26) {
            ...
            Game.log.get(m).bidding2 += ". Прикуп забирает "+name;
            Game.logger.info(". Прикуп забирает "+name);
        }
    }
